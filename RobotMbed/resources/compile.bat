...
%compiler%arm-none-eabi-g++ %ld_flags% -T %compiler_resources%/NRF51822.ld -Wl,-Map,%program_location%target/%program_name%.map -Wl,--start-group %objecs% %ld_sys_libs%--end-group -o %program_location%target/%program_name%
IF %errorlevel% NEQ 0 (
...
srec_cat %compiler_resources%/BLE_BOOTLOADER_RESERVED.hex -intel %compiler_resources%/s110_nrf51822_8.0.0_softdevice.hex -intel %program_location%target/firmware.hex -intel -o %program_location%target/%program_name%.hex -intel --line-length=44
IF %errorlevel% NEQ 0 (
