...
import de.fhg.iais.roberta.syntax.codegen.Ast2Ev3BrickPiVisitor;
...
public class BrickPiCompilerWorkflow implements ICompilerWorkflow {
    private static final Logger LOG = LoggerFactory.getLogger(BrickPiCompilerWorkflow.class);
    public final String pathToCrosscompilerBaseDir;

    public BrickPiCompilerWorkflow(String pathToCrosscompilerBaseDir) {
    ...
    public Key execute(String token, String programName, BlocklyProgramAndConfigTransformer data) {
        String sourceCode = Ast2Ev3BrickPiVisitor.generate((EV3Configuration) data.getBrickConfiguration(), data.getProgramTransformer().getTree(), true);
        try {
            storeGeneratedProgram(token, programName, sourceCode);
        } catch ( Exception e ) {
            BrickPiCompilerWorkflow.LOG.error("Storing the generated program into directory " + token + " failed", e);
    ...
    public String generateSourceCode(IRobotFactory factory, String token, String programName, String programText, String configurationText) {
        ...
        return Ast2Ev3BrickPiVisitor.generate((EV3Configuration) data.getBrickConfiguration(), data.getProgramTransformer().getTree(), true);
    }
    ...
    private void storeGeneratedProgram(String token, String programName, String sourceCode) throws Exception {
        ...
        BrickPiCompilerWorkflow.LOG.info("stored under: " + sourceFile.getPath());
