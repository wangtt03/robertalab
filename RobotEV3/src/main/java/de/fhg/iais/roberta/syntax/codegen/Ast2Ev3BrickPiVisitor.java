...
public class Ast2Ev3BrickPiVisitor extends Ast2PythonVisitor implements AstSensorsVisitor<Void>, AstActorCommunicationVisitor<Void>,
     ...
     */
    private Ast2Ev3BrickPiVisitor(EV3Configuration brickConfiguration, ArrayList<ArrayList<Phrase<Void>>> programPhrases, int indentation) {
    ...
    public static String generate(EV3Configuration brickConfiguration, ArrayList<ArrayList<Phrase<Void>>> programPhrases, boolean withWrapping) {
        ...
        Ast2Ev3BrickPiVisitor astVisitor = new Ast2Ev3BrickPiVisitor(brickConfiguration, programPhrases, 0);
    ...
    private String generateRegenerateActor(Actor actor, IActorPort port) {
        ...
        switch ( actor.getName() ) {
            ...
            case LARGE:
                ...
                break;
            default:
                throw new IllegalArgumentException("no mapping for " + actor.getName() + "to brickpi-lang-python");
        }

        sb.append("Hal.make").append(name).append("(brickpi.OUTPUT_").append(port.toString());
    ...
    private static String generateRegenerateSensor(Sensor sensor, ISensorPort port) {
        ...
        // FIXME: that won't scale
        String name = null;
        // [m for m in dir(brickpi) if m.find("_sensor") != -1]
        ...
        switch ( sensor.getType() ) {
            ...
            case ULTRASONIC:
                ...
                break;
            default:
                throw new IllegalArgumentException("no mapping for " + sensor.getType() + "to brickpi-lang-python");
        }
        sb.append("Hal.make").append(name).append("(brickpi.INPUT_").append(port.getPortNumber()).append(")");
        return sb.toString();
    }
